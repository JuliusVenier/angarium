    ...
    >
        ...
        <div id="rowID_{row.id}" class="row">
            ...
            {:else}
                ...
                <button class="btn btn-outline btn-error p-2 min-h-0 h-fit tooltip tooltip-bottom tooltip-error" data-tip="Benutzer löschen" class:btn-disabled={newUserItem_exists} on:click={deleteUser(row.id)}><Icon src={TrOutlineTrashX} size="32" /></button>
            {/if}
        </div>
    </FilterList>
...
{#if true}
    <div id="dev-tools" class="fixed right-4 top-20 flex flex-col gap-2">
        ...
        <div class="flex flex-row flex-wrap items-center justify-end gap-2">
            <span>validID</span>
            <button class="btn w-20" on:click={}>{validID}</button>
        </div>
